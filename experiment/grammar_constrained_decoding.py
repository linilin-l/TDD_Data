"""
Grammar-Constrained Decoding Experiment with GreatGramma

This script reads code samples from example_samples.jsonl and applies
...
def analyze_samples(records):
    ...
    if records:
        ...
        print(f"\nRecords by task_id:")
        for task_id, count in sorted(task_ids.items()):
...
def validate_with_lark_grammar(code: str, parser: Optional[Any]) -> tuple[bool, Optional[str]]:
    """
    ...
    if not parser or not LARK_AVAILABLE:
        return True, None  # Skip if parser not available
...
@dataclass
class JSONSchemaOutlines:
    """Represents a JSON Schema in outlines format"""
    schema: Dict[str, Any]
...
def build_json_schema_from_code(code: str, task_id: str, index: int) -> Dict[str, Any]:
    ...
    schema = {
        ...
        "properties": {
            ...
            "completion": {
                ...
                "pattern": "^[\\s\\S]*$",  # Any string including newlines
    ...
    # Add error info if syntax invalid
    if not is_valid:
...
def apply_grammar_constraints(records: List[Dict]) -> tuple[List[Dict], List[Dict]]:
    """
...
def detect_indentation(code):
    ...
    Returns:
        str: Description of indentation
    """
    lines = code.split('\n')
    for line in lines:
...
def save_results(results, output_path):
    ...
    Args:
        results (list): List of processed records
...
def save_schema_outlines(outlines, output_path):
    ...
    Args:
        outlines (list): List of schema outline records
        output_path (str): Path to output JSON file
    """
    try:
        with open(output_path, 'w', encoding='utf-8') as f:
...
def main():
    ...
    summary = {
        "experiment": "Grammar-Constrained Decoding with GreatGramma",
